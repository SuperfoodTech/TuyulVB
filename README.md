...
**Auto-OC** adalah platform otomatisasi **Auto Open & Auto Close** untuk outlet **ShopeeFood (ShopeePartner)**. Sistem ini bekerja secara cerdas mengevaluasi status outlet secara real-time berdasarkan **Vercel Toggle** sebagai *Source of Truth*, **Status Penangguhan**, **Status Subscription**, dan **Jam Operasional Outlet**.
...
- 🤖 **Automation Engine ShopeePartner**:
  - Mengatur toko secara otomatis (Buka / Tutup Sementara).
...
- 📊 **Unified Data Provider**:
  ...
  - Caching dan backup lokal berbasis **SQLite Database** ([db_manager.py](file:///mnt/DATA/Proyek/TuyulVB/common/db_manager.py)).
...
```
Auto-OC/
├── api_server.py                  # FastAPI Backend API Server
├── run.py                         # CLI Menu Interactive Runner
...
│       └── force_open/            # Refactored Force Open/Close logic & auto-sync
...
### A. Menjalankan via Docker (Rekomendasi Production)
...
### B. Menjalankan Manual (Development Mode)

1. **Jalankan Backend API Server**:
   ```bash
   python api_server.py
   ```
   *API Server akan berjalan pada `http://localhost:8000`.*

2. **Jalankan Bot Scheduler**:
   ```bash
   python run.py
   ```
   *Atau jalankan skrip master `./1. run_master.sh`.*
...
3. **Jalankan Web Frontend**:
   ...
   npm run dev
   ```
   *Aplikasi Web Dashboard akan berjalan pada `http://localhost:5173`.*

---

## 📑 Logging & Audit Trail

Sistem menyimpan setiap tindakan bot (buka store, tutup store, skip/no action, retry, error) secara terstruktur pada **SQLite Database** (`data/app.db`) dan dapat dilihat melalui tab **Audit Logs** di Web Dashboard.
