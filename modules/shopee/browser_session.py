...
class BrowserSession:
    def __init__(self, headless=True):
        ...
        try:
            ...
            # Auto-detect Chromium binary location on Linux / Raspberry Pi OS
            if sys.platform.startswith("linux"):
                for bin_path in ["/usr/bin/chromium-browser", "/usr/bin/chromium", "/usr/sbin/chromium"]:
                    if os.path.exists(bin_path):
                        options.binary_location = bin_path
                        log.info(f"Set Chromium binary location: {bin_path}")
                        break
