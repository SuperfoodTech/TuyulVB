...
- **Hybrid Architecture:** Uses Selenium _only_ for initial secure authentication, then switches to high-speed internal API calls for all store operations.
...
| **Merchant (Long)**  | Text   | The Store Name exactly as it appears in Shopee.                                        | _(Mapped in Config)_ |
| **Merchant (Short)** | Text   | A shorter name for logging/notifications.                                              | _(Mapped in Config)_ |
| **Store ID**         | Text   | **[CRITICAL]** The unique Shopee Entity ID/Store ID.                                   | _(Mapped in Config)_ |
...
This file controls _which_ merchants are processed and how they map to Monday.com columns.
...
_(Note: This runs the scheduler entry point, which triggers an immediate run upon start.)_
...
- **Fix:** Inspect the Shopee dropdown HTML and ensure the text in `settings_shopee.py` matches _exactly_.
