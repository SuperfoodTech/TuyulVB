...
def run_address_validation():
    ...
    log.info(
        f"Fetching all items from source board ID: {addressettings.SOURCE_BOARD_ID}..."
    )
    source_cols_to_fetch = list(addressettings.GROUP_MAPPING.keys())
    source_items_data = fetch_all_items_from_board(
        addressettings.SOURCE_BOARD_ID, columns=source_cols_to_fetch
    )
    ...
    for target_group_id, _ in addressettings.GROUP_MAPPING.values():
        if target_group_id not in existing_target_items_map:
            ...
            items_in_group = fetch_all_items_from_board(
                addressettings.TARGET_BOARD_ID,
                group_id=target_group_id,
                columns=[addressettings.TARGET_COL_STORE_ID],
            )
            store_id_map = {
                get_column_value(item, addressettings.TARGET_COL_STORE_ID): item["id"]
                for item in items_in_group
                if get_column_value(item, addressettings.TARGET_COL_STORE_ID)
            }
    ...
    if os.path.exists(MATCH_CACHE_FILE):
        ...
        for _, (_, match_group_id) in addressettings.GROUP_MAPPING.items():
            if match_group_id not in match_items_map:
                log.info(
                    f"Fetching items from match group '{match_group_id}' on board {addressettings.MATCH_BOARD_ID}."
                )
                items = fetch_all_items_from_board(
                    addressettings.MATCH_BOARD_ID,
                    group_id=match_group_id,
                    columns=[
                        addressettings.MATCH_COL_STORE_ID,
                        addressettings.MATCH_COL_ADDRESS,
                    ],
                )
    ...
    for i, item in enumerate(source_items_data):
        ...
        for source_col_id, (
            target_group_id,
            match_group_id,
        ) in addressettings.GROUP_MAPPING.items():
            ...
            column_values_dict = {
                addressettings.TARGET_COL_ADDRESS_STATUS: {
                    "label": status_labels.get(address_status, "False")
                },
                addressettings.TARGET_COL_ADDRESS: address_to_write,
            }
            ...
            if existing_item_id:
                ...
                update_vars = {
                    ...
                    "boardId": addressettings.TARGET_BOARD_ID,
                ...
                log.info(
                    f"  -> Creating new item on target board '{addressettings.TARGET_BOARD_ID}' in group '{target_group_id}'..."
                )
                # Only add store_id on creation
                column_values_dict[addressettings.TARGET_COL_STORE_ID] = store_id
                create_query = """
